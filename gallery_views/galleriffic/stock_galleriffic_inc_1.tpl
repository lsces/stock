...
<div class="navigation-container">
	<div id="thumbs" class="navigation">
		...
		<ul class="thumbs noscript">
			...
			<li>
				{if is_a($galItem, '\Bitweaver\Stock\StockComponent')}
					<a class="thumb" name="{$galItem->mImageId}" href="{$galItem->mInfo.thumbnail_url.large}{*$smarty.const.STOCK_PKG_URL}view_image.php?image_id={$galItem->mImageId*}" title="{$galItem->mInfo.title|escape}">
				...
				{elseif is_a($galItem, '\Bitweaver\Stock\StockAssembly')}
...
jQuery(document).ready(function($) {
	...
	var gallery = $('#thumbs').galleriffic({
		delay:                     2500,
		numThumbs:                 {/literal}{$gContent->getPreference('galleriffic_num_thumbs', $gBitSystem->getConfig('stock_gallery_default_galleriffic_num_thumbs', 30))}{literal},
		preloadAhead:              10,
		enableTopPager:            true,
...
//      onTransitionIn:            function(slide, caption, isSync) { },
		onImageLoadFinish:			function(pImageData) {
			jQuery.ajax({
				url: '{/literal}{$smarty.const.STOCK_PKG_URL}view_image_details.php?image_id={literal}'+pImageData.hash,
