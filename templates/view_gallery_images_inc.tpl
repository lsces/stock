...
{if $gContent->getLayout() == 'auto_flow'}
	...
	<div class="thumbnailblock">
		{foreach from=$gContent->mItems item=galItem key=itemContentId}
			...
			{/box}
		{foreachelse}
			<div class="norecords">This gallery is empty. <a href="{$smarty.const.STOCK_PKG_URL}upload.php?gallery_id={$gContent->mGalleryId}">Upload pictures!</a></div>
		{/foreach}
	</div>
	<div class="clear"></div>
{elseif $gContent->getLayout() == 'simple_list'}
	...
	<table class="table data">
		...
		<tr>
			...
			{/if}
			{if $gBitSystem->isFeatureActive( 'stock_item_list_size' )}
			...
			<tr class="{cycle values="odd,even"}">
				{if $thumbsize}
					<td style="text-align:center;">
						{if $galItem->mInfo.content_type_guid != 'stockassembly' }
				...
				<td>
					...
					{/if}
					{if $gBitSystem->isFeatureActive( 'stock_item_list_name' )}
						{if $br}<br />{/if}
				...
				<td class="actionicon">
					{if $galItem->mInfo.content_type_guid != 'stockassembly' }
						...
						{if $gContent->isOwner( $galItem->mInfo ) || $gBitUser->isAdmin()}
							<a href="{$smarty.const.STOCK_PKG_URL}edit_image.php?content_id={$galItem->mInfo.content_id}&amp;action=edit">{booticon iname="fa-pen-to-square" iexplain="Edit File"}</a>
							<a href="{$smarty.const.STOCK_PKG_URL}edit_image.php?content_id={$galItem->mInfo.content_id}&amp;delete=1">{booticon iname="fa-trash" iexplain="Remove File"}</a>
						{/if}
					{/if}
				</td>
			</tr>
...
{else}
	<table class="thumbnailblock">
		...
		{foreachelse}
			<tr><td class="norecords">This gallery is empty. <a href="{$smarty.const.STOCK_PKG_URL}upload.php?gallery_id={$gContent->mGalleryId}">Upload pictures!</a></td></tr>
		{/foreach}
