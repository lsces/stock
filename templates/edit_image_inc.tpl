...
{form legend="Edit Image Details"}
	...
	<div class="form-group">
		...
		{forminput}
			...
			{else}
				<img src="{$smarty.const.STOCK_PKG_URL}image/no_image.png" alt="{$gContent->getTitle()|escape}" />
			{/if}
		{/forminput}
	</div>
	...
	<div class="form-group">
		...
		{forminput}
			<label><input type="radio" name="rotate_image" value="-90"/> {biticon ipackage="stock" iname="rotate_ccw" iexplain="Rotate Counter Clockwise"}</label> &nbsp;&nbsp;&nbsp;&nbsp;
			<label>{biticon ipackage="stock" iname="rotate_cw" iexplain="Rotate Clockwise"} <input type="radio" name="rotate_image" value="90"/></label>
			<br />
	...
	<div class="form-group">
		{include file="bitpackage:stock/resize_image_select.tpl"}
	</div>
