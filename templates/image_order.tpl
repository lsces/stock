...
<div class="admin stock">
	<div class="header">
		<h1>{tr}Gallery Images{/tr}: <a href="{$smarty.const.STOCK_PKG_URL}view.php?gallery_id={$gContent->mGalleryId}">{$gContent->getTitle()|escape}</a></h1>
	</div>
		...
		{form id="batch_order" legend="Gallery Images"}
			...
			<table class="table data">
				...
				{foreach from=$gContent->mItems item=galItem key=itemContentId}
					...
					{if ($gContent->getPreference('gallery_pagination')==$smarty.const.STOCK_PAGINATION_POSITION_NUMBER && $lastMantissa != $thisMantissa) || ($gContent->mInfo.images_per_page && $imageCount % $gContent->mInfo.images_per_page == 0)}
					...
					<tr class="{$pageClass}">
						...
						<td>
							...
							{if $galItem->isContentType( STOCKCOMPONENT_CONTENT_TYPE_GUID )}
							...
							<strong>{tr}File name{/tr}</strong>: {$galItem->mInfo.filename|default:''} <br />
								...
								<strong>{tr}Edit Image{/tr}</strong>: <a href="javascript:void(0);" data-toggle="modal" data-target="#imageEditModal" data-fetch-url="{$smarty.const.STOCK_PKG_URL}edit_image.php?ajax=true&amp;content_id={$galItem->mInfo.content_id}&amp;gallery_id={$gContent->mGalleryId}">{booticon iname="fa-pen-to-square" iexplain="Edit Details"}</a>
								<noscript><div><a href="{$smarty.const.STOCK_PKG_URL}edit_image.php?content_id={$galItem->mInfo.content_id}">{booticon iname="fa-pen-to-square" iexplain="Edit Image"}</a></div></noscript>
{*								jspopup href="`$smarty.const.STOCK_PKG_URL`edit_image.php?content_id=$galItem->mInfo.content_id" title="edit image" *}
								<a target="_new" href="{$smarty.const.STOCK_PKG_URL}edit_image.php?content_id={$galItem->mInfo.content_id}">{booticon iname="fa-pen-to-square" iexplain="Edit Image"}</a>
								<br />
								{/if}
							{/if}
