<?php
/**
 * Gallery2 Remote support for stock
 *
 * @package  stock
 * @version  $Header$
...
class StockRemote {
	...
	* @param $pGalHash branch of gallery information from StockAssembly::getTree
	...
	*/
	public function traverseSubGalleries( &$pGalHash, &$pResponse, $pParentRandom ) {
		...
		foreach( $pGalHash as $key=>$gallery) {
			...
			// append pParentRandom to make .name probably unique since Stock can handle one gallery linked to multiple parents
